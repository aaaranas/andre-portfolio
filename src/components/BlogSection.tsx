      ...
      {selectedPost && (
        <div className="modal-backdrop" onClick={() => setSelectedPost(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "780px", padding: "36px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
              <div>
                ...
                <div style={{ display: "flex", gap: "16px", marginTop: "12px", fontSize: "13px", fontFamily: "var(--font-mono)", color: "var(--muted)" }}>
                  ...
                  <span>❤️ {likes[selectedPost.id] || 0} Likes</span>
                </div>
              </div>
              <button
            ...
            <div style={{ borderTop: "1px solid var(--border)", paddingTop: "24px", color: "var(--text)", fontSize: "15px", lineHeight: 1.85, whiteSpace: "pre-line" }}>
              {selectedPost.content}
            </div>

            <div style={{ marginTop: "32px", paddingTop: "20px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "16px" }}>
              <div style={{ display: "flex", gap: "8px" }}>
                {selectedPost.tags.map((tag) => (
                  <span key={tag} style={{ fontSize: "12px", fontFamily: "var(--font-mono)", color: "var(--accent2)", background: "rgba(167, 139, 250, 0.1)", padding: "4px 10px", borderRadius: "6px" }}>
                    #{tag}
                  </span>
                ))}
              </div>
